"""
Extratropical Transition Detection
==================================
Match TRV tracks with BST data to detect extratropical transition events.
"""
...
from .readers import CMABSTDataParserFixed, TCRLParser


class ExtratropicalTRVMatcher:
    """Match extratropical typhoons with TRV data and add transition flags."""
    ...
    def add_extratropical_flag_to_trv(self, match_results: pd.DataFrame, output_file: str):
        """Add extratropical transition flag to TRV file."""
            ...
            if row['has_perfect_match'] and row['perfect_matches']:
                for match in row['perfect_matches']:
                    ...
                    if key not in trv_extratropical_info:
                        ...
                        print(f"  Found matching TRV: {trv_header.name} (start: {trv_header.start_date[:8]}), "
                              f"extratropical point index: {extratropical_index}")
    ...
    def match_and_add_flag(self, bst_directory: str, trv_file_path: str, output_dir: str):
        ...
        output_file = os.path.join(output_dir, f"{base_name}_with_transition.csv")
